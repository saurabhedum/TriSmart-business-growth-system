...
import { saveUploadedData } from "../lib/db";
...
export function DataUploadView() {
  ...
  const openStagingFromParsedData = () => {
    const records = parsedData.map(row => {
      let name = String(row['Name'] || row['name'] || row['Lead Name'] || row['lead'] || row.raw || "").trim();
      let mobile = String(row['Mobile'] || row['mobile'] || row['Phone'] || row['Mobile Number'] || row['phone'] || "0000000000").trim();
      let status = String(row['Status'] || row['status'] || "New").trim();
      let source = String(row['Source'] || row['source'] || "Imported").trim();
    ...
    }).filter(c => c.name && c.name !== "undefined" && c.name !== "");

    if (records.length === 0) {
       setStatus({ type: 'error', message: 'No valid leads could be extracted' });
       return;
    }
    ...
    try {
      ...
      for (let i = 0; i < stagingCustomers.length; i += batchLimit) {
        ...
        for (const customer of chunk) {
          const docRef = doc(db, 'leads', customer.id);
      ...
      setStatus({ type: 'success', message: `Successfully imported ${stagingCustomers.length} leads.` });
              ...
              <div className="space-y-4 pt-4 border-t border-[var(--shadow-dark)]">
                ...
                <div className="flex flex-col sm:flex-row gap-3 pt-4">
                  ...
                  <button 
                    ...
                    disabled={isUploading}
                    className="flex-1 py-3 bg-accent text-white rounded-xl font-bold shadow-lg shadow-accent/30 hover:bg-blue-700 transition-colors disabled:opacity-50 flex items-center justify-center gap-2"
                  >
                    {isUploading ? <Loader2 className="w-5 h-5 animate-spin" /> : null}
          ...
          >
            ...
            <div className="flex flex-col sm:flex-row justify-between items-center mt-6 pt-4 border-t border-black/10 gap-4">
               ...
               <div className="flex gap-4 w-full sm:w-auto">
                  ...
                  <button id="btn-confirm-upload" onClick={confirmBulkUpload} disabled={isUploading} className="flex-1 sm:flex-none px-6 py-2 bg-accent text-white rounded-xl font-bold shadow-lg shadow-accent/30 flex justify-center items-center gap-2">
